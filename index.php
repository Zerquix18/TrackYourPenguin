...
?>
	...
	while($q->fetch = mysql_fetch_array($fetch) ) {
		...
		echo sprintf(
				...
				url() . IMG . $q->fetch['img'],
			__('Imagen de ') . ucwords( $q->fetch['personaje'])
		);
?>
	<center>
		...
		<?php if( es_admin() ) : ?>
		| <a href="<?php echo url() . '?accion=editar&id=' . $q->fetch['id'] ?>" title="<?php echo sprintf( __('Editar el tracker de %s'), ucwords($q->fetch['personaje'] ) )?>" class="btn btn-warning"><i class="icon-edit"></i>&nbsp;<?php _e("Editar")?></a>
		<?php endif ?>
		<hr><br>
<?php
	}
