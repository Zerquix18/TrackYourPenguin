...
class actualizacion {
	...
	public $host = 'http://trackyourpenguin.com/d/';
	...
	public function __construct() {
		...
		if( ! class_exists("ZipArchive") ) {
			$this->error = "No existe la clase <b>ZipArchive</b> para la descompresión de los archivos";
			return false;
		}

		if( ! function_exists("curl_init") || ! function_exists("curl_close" ) ) {
			$this->error = "No existe <b>curl_init</b> para la descarga de los archivos";
			return false;
		}
		...
		* En cambio lo uso porque si el archivo no existe, lo crea, y esto es lo últil.
		* Crea el archivo, que será reemplazado por el ya descargado.
		*
		*
		**/
		echo "Creando archivo temporal...<br>"; // mensaje de aviso.
		$arch = @touch( $this->archivo );
		if( ! $arch ) {
		...
		*
		**/
		$this->f = fopen( $this->dir . $this->archivo, "w" );
		...
		**/
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS, $params ); //manda el tipo...! ;o 
		curl_setopt($ch, CURLOPT_FILE, $this->f ); // donde guardará el archivo.
		echo "Descargando archivos desde " . $this->host . $this->archivo . "<br>";
		...
		if( false !== ( $this->zip->open( $this->archivo ) ) ) { // abre zip si no resulta false.
			// borra los archivos que no pueden descomprimirse...
			$this->zip->deleteName("typ-config-sample.php");
			$this->zip->deleteName("img/");
			############################################ OLÉ!
			echo "Descomprimiendo zip ya descargado... <br>";
			$this->zip->extractTo( $this->dir ) or die("No se puede descomprimir :("); // extrae al dir...
			// intenta cambiar el modo de los archivos en esa raíz...
			$this->cambiar_modo();
			// cierra zip...
			$this->zip->close();
			echo "Borrando archivo temporal...<br><br>";
			// borra el zip descargado...
			@unlink( $this->archivo );
			echo "Actualizando TrackYourPenguin...<br>";
			sleep(2); // a dormir :3
			...
			require_once( PATH . 'typ-load.php');
			agregar_info(
					sprintf(
							"Bienvenido a TrackYourPenguin <b>%s</b> :)",
							constant( "VERSION" )
						)
				);
			echo redireccion( url( true ), 3); // GUALÁAAAAAAA!!!!!!!!1 SOY EL PUTO AMOOOOOOOOo!!!!!!!!!!!D ASLDKALÑJ
		}else{
			$this->error = "No se puede abrir el archivo " . $this->archivo;
			return false;
		}
	}
}
...
function actualizaciones() {
	...
	if( ! $comparar && ! es('actualizaciones.php') )
		return agregar_error(
				sprintf(
						'TrackYourPenguin <b>%s</b> ya está disponible, por favor <a target="_blank" href="%s">actualiza.</a> :)',
						$v,
