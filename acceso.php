...
switch( $acc ) {
	...
	if( $post ) {
		...
		if( ! comprobar_args( @$_POST['email'] ) ) {
			...
			agregar_error( __("El email que buscas no existe"));
		}else{
...
<form method="POST" class="form-signin" action="<?php echo url( true ) ?>">
  ...
  <input type="text" name="email" class="input-block-level" placeholder="<?php _e('Ingresa tu email') ?>" id="email" 
...
<form method="POST" class="form-signin" action="<?php echo url( true ) ?>">
  ...
  <center><input type="submit" value="<?php _e('Actualizar clave') ?>" class="btn btn-primary"></center>
	...
	if( isset($_GET['continuar']) && is_string($_GET['continuar']) && !empty( $_GET['continuar']) ) {
